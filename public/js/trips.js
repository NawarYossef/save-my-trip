...
class Trips  {
  ...
  renderTrips(data) {
    ...
    $(".trips-container").append(allTrips)
  }
  ...
  changeArrowDirection(id) {
    $(".trips-container").on('click', `#${id}`, function() {
      console.log("EREre")
      if ($(this).hasClass("slide-down")) {
        $(this).find(".arrow-wrapper i").removeClass("fa-caret-down");
        $(this).find(".arrow-wrapper i").addClass("fa-caret-up");
      } else {
        $(this).find(".arrow-wrapper i").removeClass("fa-caret-up");
        $(this).find(".arrow-wrapper i").addClass("fa-caret-down");
      }
    });
  }

  slideDownAndShowDetails(id) {
    $(".trip-container").on('click', `#${id}`, function() {
      if ($(this).hasClass("slide-down")) {
        $(this).removeClass("slide-down");
        $(this).addClass("slide-up");
        $(this).find(".arrival-wrapper").slideUp({duration: 450});
        $(this).find(".departure-wrapper").slideUp({duration: 450});  
      } else {
        $(this).removeClass("slide-up");
        $(this).addClass("slide-down");
        $(this).find(".arrival-wrapper").slideDown({duration: 450});
        $(this).find(".departure-wrapper").slideDown({duration: 450});
        // this.changeArrowDirection();
      }
    })
  }
